  ...
  return (
    <div
      ...
      className={`cursor-pointer w-12 h-5 ${
        darkSide ? "bg-gray-100" : "bg-gray-900"
      } rounded-full relative px-1.5 flex items-center${
        darkSide ? "" : " justify-end"
      }`}
    >
      <div
        className={`w-4 h-4 rounded-full absolute transform duration-200 ease-out ${
          darkSide ? "bg-gray-900" : "bg-gray-100"
        } left-0.5 ${darkSide ? "translate-x-7" : "translate-x-0"}`}
      />
      <RenderIf
        isTrue={darkSide}
        fallback={<SunSvg className="h-3 w-3 text-gray-100" />}
      >
