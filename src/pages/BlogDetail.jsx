...
function BlogDetail() {
  ...
  useEffect(() => {
    if (!BlogsData.loading) {
      const filteredBlog = BlogsData.blogs.find(
  ...
  useEffect(() => {
    if (like) {
      dispatch(
        updateBlog(blog.id, { ...blog, reactions: blog.reactions + 1 }),
      );
    }
  }, [like]);
  ...
  return BlogsData.loading || UserData.loading || CommentData.loading ? (
    ...
    <div className="h-full pb-14">
      ...
      <div className="mx-10 bg-gray-200 rounded-bl-lg rounded-br-lg">
        ...
        <Link to="/create-blog" state={blog}>
          <EditOutlined className="float-right pr-5" />
        </Link>
      ...
      <div className="mx-10">
        {comments.map((comment) => (
          <Comment commentDetails={comment} key={comment.id} />
        ))}
      </div>
      <div className="mx-10 bg-gray-200 border-gray-400 py-5 px-3 h-24 rounded-bl-lg rounded-br-lg">
        <input
          ...
          className="w-full rounded-sm border border-gray-400"
          name="Comment"
          id="comment"
        ...
        <button
          ...
          className="text-white mt-2 float-right bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-sm text-xs px-3 py-1.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800"
        >
          Add
        </button>
      </div>
